...
if command -v nvidia-smi &> /dev/null; then
    ...
    if [ $NUM_GPUS -eq 0 ]; then
        echo "Error: No GPUs detected. This script requires GPUs to run."
        exit 1
    fi
else
...
# Note: This script was designed for 8xH100 GPUs (~4 hours runtime)
# With fewer GPUs, the runtime will scale proportionally
if [ $NUM_GPUS -lt 8 ]; then
    echo "Warning: Script was designed for 8 GPUs. With $NUM_GPUS GPU(s), runtime will be approximately $(echo "scale=1; 8 / $NUM_GPUS" | bc)x longer."
fi
